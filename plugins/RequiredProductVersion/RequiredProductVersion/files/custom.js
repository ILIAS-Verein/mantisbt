...
if( $("label:contains('Also in Version')").length ) {
    ...
    $(selector_product_version).attr('data_check_val', "").attr('data_group', 'data_product_version');
    ...
    if ($(selector_also_in_version).length > 1 || ( $(selector_product_version).val() != "" && $(selector_product_version).val() != "n.a." ) ) {
        copy = $(selector_product_version).clone();
        if(! copy.find('option[value=""]').length ){
            copy.prepend('<option value=""></option>');
        }
    ...
    if( $("input[name='" + name_also_in_version + "_presence']").length ){
        $(selector_product_version).parent().append($("input[name='" + name_also_in_version + "_presence']"));
    }
...
if( $("label:contains('Also fixed in Version')").length || $( "th.category:contains('Also fixed in Version')" ).parent().find("select").length) {
    name_also_fixed_in_version = $("label:contains('Also fixed in Version')").attr("for");

    if( name_also_fixed_in_version == null ){
        name_also_fixed_in_version = $( "th.category:contains('Also fixed in Version')" ).parent().find("select").attr("id");
    }
    ...
    $(selector_fixed_in_version).attr('data_check_val', "").attr('data_group', 'data_fixed_in_version');
    ...
    if ($(selector_also_fixed_in_version).length > 1 || ( $(selector_fixed_in_version).val() != "" && $(selector_fixed_in_version).val() != "n.a." ) ) {
        copy = $(selector_fixed_in_version).clone();
        if(! copy.find('option[value=""]').length ){
            copy.prepend('<option value=""></option>');
        }
    ...
    if( $("input[name='" + name_also_fixed_in_version + "_presence']").length ){
        $(selector_fixed_in_version).parent().append($("input[name='" + name_also_fixed_in_version + "_presence']"));
    }
    ...
    $(selector_fixed_in_version).change(function (){
        ...
        if($( this ).val() == "n.a." || $( this ).val() == ""){
            ...
            if( !last.length || last.val() != "" ){
                ...
                $( this ).parent().append("</br><label>Also fixed in Version: </label>").append(copy);
            }
        }
